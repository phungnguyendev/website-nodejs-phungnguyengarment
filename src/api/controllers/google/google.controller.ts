...
const NAMESPACE = 'controllers/google'

export default class GoogleController {
  constructor() {}
  googleDriveUploadFile = async (req: Request, res: Response) => {
    try {
      const file = req.file
  ...
  googleDriveGeneratePublicUrl = async (req: Request, res: Response) => {
    try {
  ...
  googleDriveDeleteFile = async (req: Request, res: Response) => {
    try {
      const fileId = req.params.fileId
      if (!fileId) return res.formatter.serverError({ message: 'Internal error!' })
